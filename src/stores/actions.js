...
export const updateCheck = ({commit}, {item, checked, isAdd}) => {
	// TODO: Call service
	commit('UPDATE_CHECK', {item, checked});
	if (isAdd) {
		let message_obj = {
		  message: `Add ${item.title} to cart successfully`,
		  messageClass: "success",
...
export function listenToProductList({commit}) {
	return ref.child("products").on('value', (hotel) => {
		commit('UPDATE_PRODUCT_LIST', hotel.val());
	});
}

export function getShoppingCart({commit}, { currentCart}) {

		return ref.child("cart/").once('value').then((cart) => {
...
export function saveShoppingCart({commit}, { cartItemList}) {
	// console.log("ACTIONS saveShoppingCart");
	// console.log("CART DATA", cartItemList);
	return ref.child("cart/").set(cartItemList);
}
